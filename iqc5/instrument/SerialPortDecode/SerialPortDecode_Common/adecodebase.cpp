...
int ADecodeBase::CutAstmCheckSum(QString &as_data) const
{
    ...
    li_pos = as_data.indexOf(QChar(2));
    ...
    li_pos = as_data.indexOf(QChar(23));
    ...
    li_pos = as_data.indexOf(QChar(3));
...
QDateTime ADecodeBase::GetDateByFormat(QString as_str, QString as_format)
{
    qint32 li_pos;
    qint32 li_year;
    qint32 li_month;
    ...
    as_format = as_format.trimmed();
    as_str = as_str.trimmed().toUpper();

    if (as_str.length() < as_format.length())
    {
        return ldt_datetime.currentDateTime();
    }

    li_pos = as_format.indexOf("YYYY");

    if (li_pos >= 0)
    {
        li_year = as_str.mid(li_pos,4).toInt();
    }
    else
    {
        li_pos = as_format.indexOf("YY");

        if (li_pos >= 0)
        {
            li_year = as_str.mid(li_pos,2).toInt();
        }
        else
        {
            li_year = ldt_date.currentDate().year();
        }
    }
    ...
    li_pos = as_format.indexOf("MM");
    if (li_pos >= 0)
    {
        li_month = as_str.mid(li_pos,2).toInt();
    }
    else
    {
        li_month = ldt_date.currentDate().month();
    }

    li_pos = as_format.indexOf("DD");
    if (li_pos >= 0)
    {
        li_day = as_str.mid(li_pos,2).toInt();
    }
    else
    {
        li_day = ldt_date.currentDate().day();
    }

    li_pos = as_format.indexOf("hh");
    if (li_pos >= 0)
    {
        li_hour = as_str.mid(li_pos,2).toInt();
    }
    else
    {
        li_hour = ldt_time.currentTime().hour();
    }

    li_pos = as_format.indexOf("mm");
    if (li_pos >= 0)
    {
        li_min = as_str.mid(li_pos,2).toInt();
    }
    else
    {
        li_min = ldt_time.currentTime().minute();
    }

    li_pos = as_format.indexOf("ss");
    if (li_pos >= 0)
    {
        li_sec = as_str.mid(li_pos,2).toInt();
    }
    else
    {
        li_sec = ldt_time.currentTime().second();
    }

    ls_datetime = QString("%1-%2-%3 %4:%5:%6")
            .arg(QString::number(li_year).rightJustified(4,'0'))
            .arg(QString::number(li_month).rightJustified(2,'0'))
            .arg(QString::number(li_day).rightJustified(2,'0'))
            .arg(QString::number(li_hour).rightJustified(2,'0'))
            .arg(QString::number(li_min).rightJustified(2,'0'))
            .arg(QString::number(li_sec).rightJustified(2,'0'));

    ldt_datetime = QDateTime::fromString(ls_datetime);
...
int ADecodeBase::GetSeg(QString as_str, QStringList &asl_list)
{
    ...
    while (li_pos >= 0)
    {
        asl_list.insert(li_ret, as_str.left(li_pos));
        as_str = as_str.mid(li_pos + 1);
        li_ret++;
...
QString ADecodeBase::reverseStr(QString as_str)
{
    QString ls_ret;
    qint32 li_len;
    if (as_str.isNull())
