...
W_5700::W_5700(QWidget *parent)
    : BaseWindow(parent)
{
    this->setWindowTitle(tr("PCR_5700"));
    setFileSuffix("*.csv");
...
int W_5700::file_decode(const QString &filepath)
{
    ...
    if(lsl_list.size() == 1)
    {
        if (m_nWellIndex > lsl_list.at(0).split(",").size())
        {
            return 1;
        }
    }
    ...
    for(int i= 0; i < lsl_list.size(); i++)
    {
        ...
        if (m_nWellIndex <= lsl_unit.size())
        {
            ...
            if (li_row > 0 && li_row <= 8)
            {
                ...
                {
//                    li_seq = li_row + li_col * 100;   //不用序列号
                    if (!ls_result.isEmpty() && !ls_result.trimmed().isEmpty())
                    {

//                        ls_result = m_dec.GetResult(ls_result);
                    }
                    else
                    {
                        ls_result = "none";
...
void W_5700::loadConfig()
{
    BaseWindow::loadConfig();

    m_nResultIndex = readSetting("5700/resultidx",7).toInt();
    m_nWellIndex = readSetting("5700/wellidx",1).toInt();

}

void W_5700::set_TableWidgetHeader()
{
...
void W_5700::on_Import_PushButton_clicked()
{
    QString ls_sampleNo;
    QString ls_itemNo;
    QString ls_result;
