...
QString g_sItemTables_file = QDir::toNativeSeparators(dir.currentPath().append("/ItemTables.txt"));

QString g_sRegInstr = "";

QString g_sInstallPlace = "";
...
void f_loadConfig()
{
    ...
    g_sInstallPlace = readSetting("INSTR/installplace","").toString();
    g_sHostName = readSetting("SERVER/hostname","127.0.0.1").toString();
    g_nHostPort = readSetting("SERVER/port",3306).toInt();
    g_sSqlPath = readSetting("databasepath","../../../mytest.db").toString();
    g_sSqlPath = QDir::toNativeSeparators(g_sSqlPath);
    writeSetting("databasepath",g_sSqlPath);
}
